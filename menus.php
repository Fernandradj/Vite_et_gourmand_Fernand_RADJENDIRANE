    ...
    <main>
        <div class="filter_section">
            ...
            <!-- <div>
                ...
                <form action="recherche.php" method="GET" style="max-width: 400px; margin: 20px;">
                    <h3>Prix</h3>

                    <div id="price-slider" style="margin-bottom: 20px;"></div>

                    <div style="display: flex; justify-content: space-between;">
                        <span>Prix : <strong id="price-label">0€ - 1000€</strong></span>
                        <input type="hidden" name="min_price" id="min_price" onchange="prixMin()">
                ...
                <script>
                    ...
                    noUiSlider.create(slider, {
                        start: [100, 500], // Valeurs par défaut au chargement
                        connect: true, // Relie les deux curseurs
                        range: {
                            'min': 0,
                            'max': 1000
                        },
                    ...
                    // Mise à jour des champs lors du glissement
                    slider.noUiSlider.on('update', function (values, handle) {
                        minInput.value = values[0];
                        maxInput.value = values[1];
                        priceLabel.innerHTML = values[0] + "€ - " + values[1] + "€";
                    });
                </script>
            </div> -->
            <div class="pick_list_container">
                <label class="theme_filter" for="prix-select">Prix</label>
                <input type="number" id="prix-minimum" onchange="refresh()" class="theme_select_filter" name="prix_min"
                    value="0" min="0" max="1000" step="1"
                    onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57">
                <input type="number" id="prix-maximum" onchange="refresh()" class="theme_select_filter" name="prix_max"
                    value="1000" min="0" max="1000" step="1"
                    onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57">
            </div>

            <div class="pick_list_container">
                <label class="theme_filter" for="theme-select">Thème</label>

                <select id="theme-select" class="theme_select_filter" name="theme" onchange="refresh()">
                    <option value="">-- Veuillez choisir --</option>
                    <option value="Noêl">Noël</option>
            ...
            <div class="pick_list_container">
                <label class="theme_filter" for="personne-select">Personne Minimum</label>
                <input type="number" id="personne-select" onchange="refresh()" class="theme_select_filter"
                    name="personne_min" value="1" min="1" max="10" step="1"
                    onkeypress="return (event.charCode == 8 || event.charCode == 0 || event.charCode == 13) ? null : event.charCode >= 48 && event.charCode <= 57">
            </div>
            <div class="pick_list_container">
                <label class="theme_filter" for="regime-select">Régime</label>

                <select id="regime-select" class="theme_select_filter" name="regime" onchange="refresh()">
                    <option value="">-- Veuillez choisir --</option>
                ...
                <div class="solution_cards_box" id="menuBox">
                    ...
                    if (!empty($menu_list)) {
                        foreach ($menu_list as $menu) {
                            echo '<div class="solution_card" id="' . $menu->getId() . '">';
                            echo '<div class="hover_color_bubble"></div>';
                            echo '<div class="so_top_icon">';
                            echo '<svg id="Layer_1" enable-background="new 0 0 512 512" height="50"
                            ...
                            echo '</div>';
                            echo '<div class="solu_title">';
                            echo '<h3>' . $menu->getNom() . '</h3>';
                            echo '</div>';
                            echo '<div class="solu_description">';
                            echo '<p>' . $menu->getDescription() . '</p>';
                            echo '<p><b>Nombre de personne minimum</b> : </p>';
                            echo '<p class="personne_min">' . $menu->getNombre_personne_minimum() . '</p>';
                            echo '<p><b>Prix par personne</b> : </p>';
                            echo '<p class="prix_personne">' . $menu->getPrix_par_personne() . '</p>';
                            echo '<p><b>Régime </b>: </p>';
                            echo '<p class="regime">' . $menu->getRegime() . '</p>';
                            echo '<p><b>Thème </b>: </p>';
                            echo '<p class="theme">' . $menu->getTheme() . '</p>';

                            echo '<a href="detail_menu.php?menuId=' . $menu->getId() . '"> <button type="button" class="read_more_btn">Détail</button></a>';
                            echo '</div>';
                            echo '</div>';
                        }
                    }
                ...
                <div id="carouselExampleSlidesOnly" class="carousel slide" data-bs-ride="carousel">
                    <div class="carousel-inner">
                        <div class="carousel-item active" data-interval="1000">
                            <img src="./images/calcul img.webp" class="d-block w-10" alt="...">
                        </div>
                        <div class="carousel-item" >
                            <img src="./images/avion.jpg" class="d-block w-10" alt="...">
                        </div>
                        <div class="carousel-item" >
                            <img src="./images/calcul img.webp" class="d-block w-10" alt="...">
                        </div>
